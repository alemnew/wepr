...
int dns_detect_domain_loop(DNSCache *cache, unsigned int domain_trail[],
		char *qname) {
	DNSMessage *message;
	int i = 0;
...
int dns_resolve(DNSCache *cache, DNSResolverState *state, char *qname_original,
		DNSRecordType qtype, char **final_qname) {
	...
	while (queue_top != NULL && state->nrof_responses < DNS_MAX_RESPONSES) {
		...
		if ((final_rr = dns_cache_find_rr(cache, queue_top->qname, qtype,
			...
			if (final_rr->type == qtype) {
				...
				free(debug_str);
#endif
				state->responses[state->nrof_responses++] = dns_message_copy(
						final_msg, state->nwp); /* Resolver MUST always return COPY of entries in cache. */
			...
			for (i = 0; i < best_authority->nrof_authorities; i++) {
				...
				if (dns_cache_find_rr(cache, ns->data, A_AAAA,
						ANSWERS | ADDITIONALS, &best_destination) == NULL) {
					if (state->recurse) {
				...
				nrof_best_destinations = dns_message_nwp(best_destination,
						state->nwp, &best_destination_nwp);
				...
				for (j = 0; j < nrof_best_destinations; j++) {
					...
					if (destination->type == A) {
						...
						memcpy(
								...
								destination->data, 4); /* Copy IPv4 address. */
						...
						memcpy(
								...
								destination->data, 16); /* Copy IPv6 address. */
					...
					if ((state->queries[state->nrof_queries] = calloc(1,
						...
						if (send(sock, qpacket, qpacket_len, 0)
							...
							case 1:
								...
								if ((respbuf_len = recv(sock, respbuf,
										sizeof(respbuf), MSG_DONTWAIT)) != -1) {
									/* Set reception time. */
									gettimeofday(&tm_end, NULL);

									/* Quickly check transaction ID. */
									rcvd_id = ntohs(chars_to_short(respbuf));
									if (rcvd_id == qpacket_id) {
										...
										last_response->rtt = timeval_to_msec(
												&tm_diff);
										last_response->pksize = respbuf_len;
										/* Parse response. */
										if ((parser_retval = dns_message_parse(
												state, cache, respbuf,
												respbuf_len, queue_top->qname))
												== DNS_OK) {
...
char *dns_record_rdata_str(DNSRecord *record) {
	...
	if (record->type == A || record->type == AAAA) {
		...
		snprintf(result, 1024,
				...
				soa->domain, soa->serial, soa->refresh, soa->retry, soa->expire,
				soa->minimum_ttl);
	} else {
...
DNSResolverState *dns_state_init() {
	...
	state->recursive_authority.label = allocstrcpy(DNS_FAKE_ROOT_SERVER_NAME,
			strlen(DNS_FAKE_ROOT_SERVER_NAME), 1);

	/* Create NS record for recursive resolver. */
	if ((state->recursive_authority.authorities[0] = calloc(1,
			sizeof(DNSRecord))) == NULL) {
		free(state);
		return NULL;
	}
