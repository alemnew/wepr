...
void cdn_detect(ElementStat *stat, DNSResolverState *dns_state);

void stat_set_dns_trigger(HURLPath *path, HURLConnection *connection) {
	...
	if (stat->dns_trigger != NULL) {
		...
		log_debug(__func__, "DNS trigger already set: %s", trigger_stat->url_hash);
		return;
	}
	...
	if (strcmp(trigger_stat->url_hash, stat->url_hash) == 0) {
		log_debug(__func__, "SKIP-HASH: '%s' != '%s'", trigger_stat->url, stat->url);
	...
	if (trigger != path) {
		...
		log_debug(__func__, "#DNS %s RESOLVED BY %s ", stat->url_hash, trigger_stat->url_hash);
	}
}

int stat_redirect(HURLPath *path, int response_code, char *redirect_url) {
	ElementStat *stat = (ElementStat *) path->tag;
...
int stat_pre_connect(HURLPath *path, HURLConnection *connection) {
	ElementStat *stat = (ElementStat *) path->tag;
	...
	log_debug(__func__, "Began connecting @ %f %s%s", timeval_to_msec(&stat->http->begin_connect), path->server->domain->domain, path->path);
	/* Always allow connect */
	return 1;

}
void stat_post_connect(HURLPath *path, HURLConnection *connection, int retval) {
	ElementStat *stat = (ElementStat *) path->tag;
...
int stat_send_request(HURLPath *path, HURLConnection *connection, int pipelined) {
	ElementStat *stat = (ElementStat *) path->tag;
	/* Initialize HTTP statistics */
	if (!stat->http) {
		stat->http = calloc(1, sizeof(HTTPStat));
		log_debug(__func__, "Initialized stat->http of %s%s", connection->server->domain->domain, path->path);
	}
...
void stat_header_received(HURLPath *path, int response_code, HURLHeader *headers, size_t header_len) {
	HURLHeader *h;
	ElementStat *stat = (ElementStat *) path->tag;
	struct tm date;
	Buffer *json = NULL;
	...
	while (h != NULL) {
		...
		} else if (test->stats.http.all_headers || hurl_header_exists(test->stat_headers, h->key)) {
			escaped = json_escape(h->value);
			tmp_len = strlen(escaped) + strlen(h->key) + strlen("\"\":\"\",") + 1;
		...
		if (strcasecmp("content-type", h->key) == 0) {
			stat->http->content_type = allocstrcpy(h->value, strlen(h->value), 1);
		}
		h = h->next;
...
void cdn_detect(ElementStat *stat, DNSResolverState *dns_state) {
	...
	for (i = dns_state->nrof_responses - 1; i >= 0; i--) {
		...
		for (j = 0; j < r->nrof_answers; j++) {
			...
			if (a->type == CNAME) {
				if (strstr(a->data, ".edgesuite.net") || strstr(a->data, "g.akamai.net")) {
					/* Akamai detected */
					*cdn = CDN_AKAMAI;
					domain->tag = cdn;
...
void dns_resolve_wrapper(HURLDomain *domain, HURLPath *path) {
	int retval, i;
	...
	log_debug(__func__, "Resolution triggered by %s%s", domain->domain, path->path);
	...
	if (inet_pton(AF_INET, domain->domain, &((struct sockaddr_in *) domain->addresses[0])->sin_addr) == 1) {
		/* The string was actually an IPv4 address */
		nrof_addresses++;
		((struct sockaddr_in *) domain->addresses[0])->sin_family = AF_INET;
	} else if (inet_pton(AF_INET6, domain->domain, &((struct sockaddr_in6 *) domain->addresses[0])->sin6_addr) == 1) {
	...
	retval = dns_resolve(test->cache, &state, domain->domain, test->dns_query_type, &final_qname);
	...
	if (retval == DNS_OK) {
		...
		domain->nrof_addresses = dns_count_rr(test->dns_query_type, ANSWERS, response);
		...
		for (i = 0; i < response->nrof_answers; i++) {
			...
			if (record->type == A) {
				domain->addresses[nrof_addresses] = calloc(1, sizeof(struct sockaddr));
				domain->addresses[nrof_addresses++]->sa_family = AF_INET;
				memcpy(&((struct sockaddr_in *) domain->addresses[i])->sin_addr.s_addr, record->data, record->data_len);
			} else if (record->type == AAAA) {
				domain->addresses[nrof_addresses] = calloc(1, sizeof(struct sockaddr));
				domain->addresses[nrof_addresses++]->sa_family = AF_INET6;
				memcpy(&((struct sockaddr_in6 *) domain->addresses[i])->sin6_addr, record->data, record->data_len);
			}
		}
	...
	if (retval == 0) {
		...
		if ((record = dns_message_find_rr(response, ANSWERS, final_qname, A)) != NULL) {
		...
		if ((record = dns_message_find_rr(response, ANSWERS, final_qname, AAAA)) != NULL) {
...
char *path_filename(HURLPath *path) {
	...
	for (i = 1; i < path_len; i++) {
		if (path->path[i] == '/' || isspace(path->path[i]) || isblank(path->path[i])) {
			filename[j++] = '-';
		} else {
 ...
 */
void stat_body_recv(HURLPath *path, char *data, size_t data_len) {
	char *filename;
	int filename_len;
	ElementStat *stat = (ElementStat *) path->tag;
	...
	if (test->stats.http.save_body) {
		if (stat->fp == 0) {
			filename_len = strlen(test->body_path) + 1 + strlen(stat->url_hash) + strlen(".body") + 1;
			if ((filename = malloc(sizeof(char) * filename_len)) != NULL) {
				snprintf(filename, filename_len, "%s/%s.body", test->body_path, stat->url_hash);
				/* Open output file */
				if ((stat->fp = open(filename, O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR)) == -1) {
					log_debug(__func__, "Failed to open file '%s': %s", filename, strerror(errno));
					free(filename);
		...
		if (data_len > 0) {
			...
			if (write(stat->fp, data, data_len) != data_len) {
				log_debug(__func__, "Failed to write to '%s': %s", stat->url_hash, strerror(errno));
			}
		} else {
			/* Close file */
			close(stat->fp);
...
void stat_response_code(HURLPath *path, HURLConnection *connection, int response_code, char *response_code_text) {
	ElementStat *stat = (ElementStat *) path->tag;
	/* Initialize HTTP statistics */
	if (!stat->http) {
		stat->http = calloc(1, sizeof(HTTPStat));
...
void stat_response_latency(HURLPath *path, HURLConnection *conn, char *data, size_t data_len) {
	char *eof_header;
	int first_recv = 0;
#ifndef NDEBUG
	...
	if (stat->http->bgof_header == 0) {
		...
		log_debug(__func__, "First header bytes received after %f ms.", stat->http->bgof_header);
	}
	if (stat->http->bgof_body == 0 && stat->http->header_len > 0) {
		if (stat->http->header_len < data_len) {
			...
			log_debug(__func__, "First body bytes received after %f ms.", stat->http->bgof_body);
		}
	}
}

/* TODO: I don't think this function is being used at all */
void stat_transfer_failed(HURLPath *path, HURLConnection *conn, size_t content_len, size_t overhead) {
	ElementStat *stat = (ElementStat *) path->tag;
	gettimeofday(&stat->end_transfer, NULL);
}
