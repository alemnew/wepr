...
void *duplicate_element_stat(HURLPath *new_path, HURLPath *redirector,
	...
	element_url_hash(stat);

	if (hurl_parse_url(destination_url, &parsed_url)) {
		if (test->stats.http.tls || test->stats.http.domain
				|| test->stats.http.port || test->stats.http.path) {
...
void element_url_hash(ElementStat *element) {
	int h;
	SHA_CTX sha_context;
